...
export default function Home() {
  ...
  const { wallets, transactions, createWallet, addMoney } = useWallets();
  ...
  const handleCreate = (data) => {
    const w = createWallet(data);
    pushToast(`Wallet "${w.name}" created`);
  };
  ...
  const userInitials = (auth?.user?.name || auth?.user?.username || 'U')
    .split(' ').map((s) => s[0]).slice(0, 2).join('').toUpperCase();

  const navItems = [
    { id: 'home',     label: 'Overview',     icon: Icon.home },
    { id: 'wallets',  label: 'Wallets',      icon: Icon.wallet, badge: wallets.length },
    ...
    <div className="app-shell">
      <aside className="app-side">
          ...
          <button key={item.id} className={'nav-item' + (activeNav === item.id ? ' active' : '')}
            ...
            {item.badge != null && (
              <span style={{
                ...
                background: activeNav === item.id ? 'rgba(255,255,255,.15)' : 'var(--ink-100)',
        ...
        <div className="app-side-bottom">
          ...
          <div className="meta" style={{flex: 1, minWidth: 0}}>
            <div className="name">{auth?.user?.name || 'Guest'}</div>
            <div className="sub">@{auth?.user?.username || 'guest'}</div>
          </div>
      ...
      <main className="app-main">
        <div className="app-top">
          <div>
            <h1>Good day, {(auth?.user?.name || 'there').split(' ')[0]}.</h1>
        ...
        <div className="wallet-grid">
          {wallets.map((w) => (
            <WalletCard key={w.id} wallet={w} onClick={() => setAddMoneyFor(w)} />
          ))}
